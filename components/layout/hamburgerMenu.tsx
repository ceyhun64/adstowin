...
export default function HamburgerMenu({
  ...
  return ReactDOM.createPortal(
    <AnimatePresence>
      {isMenuOpen && (
        <>
          {/* ARKA PLAN FLULAŞTIRMA */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="fixed inset-0 bg-[#020617]/40 backdrop-blur-md z-[9998]"
          />
          ...
          <motion.aside
            ...
            className="fixed right-0 top-0 z-[9999] h-full w-full max-w-[380px] bg-[#020617] border-l border-white/10 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col"
          >
            {/* ÜST GRADYAN EFEKTİ */}
            <div className="absolute top-0 right-0 w-full h-64 bg-gradient-to-b from-indigo-600/10 to-transparent pointer-events-none" />
            ...
            <div className="relative flex flex-col h-full z-10">
              ...
              <div className="p-8 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-black tracking-[0.4em] text-slate-500 uppercase italic">
                    Kontrol Paneli
                  </span>
                </div>
                <button
                  onClick={onClose}
                  className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all hover:rotate-90"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-8 pb-12 custom-scrollbar">
                ...
                <div className="relative p-6 rounded-[28px] bg-gradient-to-br from-white/[0.05] to-transparent border border-white/10 mb-10 overflow-hidden group">
                  <div className="absolute -top-4 -right-4 w-24 h-24 bg-indigo-600/10 blur-3xl group-hover:bg-indigo-600/20 transition-all" />
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <div className="relative">
                      ...
                      <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 border-4 border-[#020617] rounded-full" />
                    </div>
                    <div>
                ...
                <div className="space-y-10">
                  <Section title="Yönetim & Profil">
                    ...
                    <button
                      ...
                      className="mt-4 w-full group flex items-center justify-center gap-3 p-4 rounded-[20px] bg-red-500/5 border border-red-500/10 hover:bg-red-500 hover:text-white transition-all duration-300"
                    >
                      <LogOut
                        size={18}
...
function Section({ title, children }: any) {
  return (
    <div>
      <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] mb-4 ml-1">
        {title}
      </h4>
...
function MenuLink({ icon: Icon, label, href, onClick }: any) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="flex items-center justify-between p-4 rounded-[18px] hover:bg-white/5 border border-transparent hover:border-white/5 transition-all group"
    >
      <div className="flex items-center gap-4 text-slate-400 group-hover:text-white transition-colors">
        <Icon
          size={18}
          className="opacity-50 group-hover:opacity-100 group-hover:text-indigo-400"
        />
